...
public class EventScheduler {
    ...
    private void scheduleNextEvent() {
        new BukkitRunnable() {
            ...
            public void run() {
                if (isActivePeriod()) {
                    int randomDelayMinutes = random.nextInt(121);
                    LocalDateTime eventTime = LocalDateTime.now().plusMinutes(randomDelayMinutes);

                    Bukkit.getLogger().info("Следующий ивент начнется через " + randomDelayMinutes + " минут (" + eventTime + ").");

                    scheduleEventAfterDelay(randomDelayMinutes * 60L);
                } else {
                    long initialDelay = calculateInitialDelay();
                    Bukkit.getLogger().info("Не в активном периоде. Ждем " + (initialDelay / 60) + " минут до следующего активного периода.");
    ...
    private void scheduleEventAfterDelay(long delaySeconds) {
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                Bukkit.getLogger().info("Ивент " + randomEvent + " запущен!");
                ...
                Bukkit.getLogger().info("Следующий ивент будет запущен через " + nextDelayMinutes + " минут (" + nextEventTime + ").");

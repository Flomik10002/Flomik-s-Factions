...
public class ClanManager {
    ...
    public void createClan(String name, String owner) {
        ...
        int level = 0; // Начальный уровень клана
        int clanXp = 0; // Начальный уровень клана
    ...
    public void saveClan(Clan clan) {
        ...
        config.set(path + ".clanXp", clan.getClanXp()); // Сохранение уровня клана
    ...
    public void loadClans() {
        if (config.contains("clans")) {
            for (String clanName : config.getConfigurationSection("clans").getKeys(false)) {
                ...
                int level = config.getInt("clans." + lowerCaseClanName + ".level", 0);
                int clanXp = config.getInt("clans." + lowerCaseClanName + ".clanXp", 0);
                ...
                Clan clan = new Clan(
                        displayName, owner, memberSet, memberRoles, creationDate, description, alliances, level, clanXp, land, strength, maxPower, chunks
                );
                clan.setHome(home);
